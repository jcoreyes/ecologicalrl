...
class SimpleReplayBuffer(ReplayBuffer):
    ...
    def __init__(
        ...
        self._actions = np.zeros((max_replay_buffer_size, action_dim), dtype=dtype)
        # Make everything a 2D np array to make it easier for other code to
        # reason about the shape of the data
        self._rewards = np.zeros((max_replay_buffer_size, 1), dtype=dtype)
        # self._terminals[i] = a terminal was received at time i
        self._terminals = np.zeros((max_replay_buffer_size, 1), dtype=dtype)
        # Define self._env_infos[key][i] to be the return value of env_info[key]
        # at time i
        self._env_infos = {}
        for key, size in env_info_sizes.items():
            self._env_infos[key] = np.zeros((max_replay_buffer_size, size), dtype=dtype)
