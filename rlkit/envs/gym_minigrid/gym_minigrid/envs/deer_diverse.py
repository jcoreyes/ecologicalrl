...
class DeerDiverseEnv(FoodEnvBase):
    """
    ...
    def __init__(
            self,
            # list of 2 dicts specifying start and end dists
            deer_dists,
            deer_dist_period,
            grid_size=32,
            health_cap=100,
        ...
        # easy medium hard deer stuff
        self.start_deer_dist, self.end_deer_dist = deer_dists
        assert sum(self.start_deer_dist.values()) == sum(self.end_deer_dist.values()) == 1,\
            'dists must be valid probability distributions'
        self.deer_dist_period = deer_dist_period
        ...
        self.actions = DeerDiverseEnv.Actions
        ...
        for type, prob in self.resource_prob.items():
            ...
            if self.place_schedule and not self.full_grid:
                ...
                if type == 'deer':
                    new_obj = TYPE_TO_CLASS_ABS[type](lifespan=self.lifespans.get(type, self.default_lifespan),
                                                      difficulty=self.get_difficulty())
                    if self.place_prob(new_obj, place_prob,
                                       top=(np.clip(self.agent_pos - diam // 2, 0, self.grid_size-1)), size=(diam, diam)):
                        self.deer.append(new_obj)
                else:
                    new_obj = TYPE_TO_CLASS_ABS[type](lifespan=self.lifespans.get(type, self.default_lifespan))
                    self.place_prob(new_obj, place_prob,
                                    top=(np.clip(self.agent_pos - diam // 2, 0, self.grid_size - 1)), size=(diam, diam))
    ...
    def get_difficulty(self):
        difficulties = list(self.start_deer_dist.keys())
        start_probs = np.array([self.start_deer_dist[k] for k in difficulties])
        end_probs = np.array([self.end_deer_dist[k] for k in difficulties])
        c = min(self.step_count / self.deer_dist_period, 1)
        probs = start_probs * (1 - c) + end_probs * c
        ...
        for type, count in self.init_resources.items():
            if self.task and self.task[0] == 'pickup' and type == self.task[1]:
                for _ in range(count):
                    if type == 'deer':
                        difficulties = list(self.start_deer_dist.keys())
                        probs = [self.start_deer_dist[k] for k in difficulties]
                        difficulty = np.random.choice(difficulties, p=probs)
                        new_obj = TYPE_TO_CLASS_ABS[type](difficulty=difficulty)
                        self.deer.append(new_obj)
                    else:
    ...
    def extra_step(self, action, matched):
        ...
        for deer in self.deer:
            if self._rand_float(0, 1) < self.deer_move_prob:
                ...
                ac = np.zeros(2)
                if deer.difficulty == 'easy':
                    ac = min(acs, key=lambda val: np.linalg.norm(np.clip(deer.cur_pos + val, 1, self.grid_size - 2) - self.agent_pos, ord=1))
                elif deer.difficulty == 'medium':
                    ac = random.choice(acs)
                elif deer.difficulty == 'hard':
                    ac = max(acs, key=lambda val: np.linalg.norm(np.clip(deer.cur_pos + val, 1, self.grid_size - 2) - self.last_agent_pos, ord=1))
                new_pos = np.clip(deer.cur_pos + ac, 1, self.grid_size - 2)
