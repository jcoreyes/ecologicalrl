...
COLOR_NAMES4 = sorted(list(COLORS4.keys()))
...
IDX_TO_COLOR4 = dict(zip(COLOR_TO_IDX4.values(), COLOR_TO_IDX4.keys()))
...
# Map of direction names to vectors
DIR_TO_VEC = {
    # Up (negative Y)
    'north': np.array((0, -1)),
    # Left (negative X)
...
class WorldObj:
    ...
    def _set_color(self, r, color=None):
...
class TriangleOverlap(WorldObj):
...
class House(WorldObj):
    def __init__(self, color='orange', lifespan=0):
        super().__init__('house', color, lifespan=lifespan)
...
class GridAbsolute:
    ...
    def rotate_left(self):
        ...
        grid = GridAbsolute(self.width, self.height)
    ...
    #             else:
    #                 v = TYPE_TO_CLASS_ABS[obj_type](color)
    #             grid.set(i, j, v)
    #
    #     return grid
    ...
    def process_vis(grid, agent_pos):
        ...
        for j in range(agent_pos[1], grid.height - 1):
            for i in range(0, grid.width - 1):
                if not mask[i, j]:
                    continue
...
class MiniGridAbsoluteEnv(gym.Env):
    """
    ...
    # Drop an object
    # drop = 5
    # NOTE: toggle not used b/c no fwd dir. toggle by moving into obj
    ...
    def reset(self):
        ...
        if hasattr(self, 'one_hot_obs'):
            obs = self.gen_obs(onehot=self.one_hot_obs)
        else:
            obs = self.gen_obs()
        return obs
    ...
    def __str__(self):
        ...
        # Map agent's direction to short string (fixed as ⏩ here since no dir)
        AGENT_DIR_TO_IDS = {
            0: '⏩ ',
        }

        array = self.grid.encode(self)
        ...
        # Add the agent (agent dir hardcoded since no dir in abs env)
    ...
    def agent_sees(self, x, y):
        ...
        obs = self.gen_obs()
        obs_grid = GridAbsolute.decode(obs['image'])
    ...
    def step(self, action, override=False):
        ...
        # 	if agent_cell and agent_cell.can_pickup():
        # 		if self.carrying is None:
        # 			self.carrying = agent_cell
        # 			#self.carrying.cur_pos = np.array([-1, -1]) # won't appear on map
        # 			self.carrying.held_pos = self.agent_pos
        # 			self.carrying.is_held = True
        # 			#self.carrying.cur_pos = self.agent_pos
        # 			self.grid.set(*self.agent_pos, None) # object disapears from grid
        # 			self.carry_flag = True
        #
        # # Drop an object
        # elif action == self.actions.drop:
        # 	if not agent_cell and self.carrying:
        # 		self.grid.set(*self.agent_pos, self.carrying)
        # 		self.carrying.cur_pos = self.agent_pos
        # 		self.carrying.is_held = False
        # 		self.carrying.held_pos = None
        # 		self.carrying = None
        # 		self.carry_flag = False
        ...
        #     if agent_cell:
        #         agent_cell.toggle(self, self.agent_pos)
        ...
        obs = self.gen_obs(onehot=getattr(self, 'one_hot_obs', False))
        reward = self._reward()
